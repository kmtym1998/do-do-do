...
### ステップ2: GitHubにリポジトリを作成(folk)しよう
...
- リポジトリのREADMEに従い、docker上で環境構築をしましょう
  - `rails new` してできたプロジェクトのディレクトリ（アプリ名のディレクトリ）の直下に `docs` というディレクトリを作り、この文書ファイルをコミットしましょう
    ...
    - READMEにある `docker-machine` のステップは基本的に必要ないはずなので飛ばしてかまいません
...
※ RailsではActiveRecord（パターン）を使用するため、基本的にモデルとテーブルが１：１になりますが、そうではないケースもあります。気になる場合は参考としてコーチに質問してみましょう
...
- `database.yml` の設定をしましょう（READMEに記載の内容）
...
- これまで出してきたPRを振り返って、テンプレートを作ってみましょう
